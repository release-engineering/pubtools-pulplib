    ...
    **Throttling:**
    ...
    def get_maintenance_report(self):
        ...
        try:
            ...
            # HTTPError 404.
            LOG.warn("No maintenance info found")
            return None
    ...
    def set_maintainenance(self, regex, enable=True, **kwargs):
        ...
        # unassociate the repos.json file first
        LOG.debug("Removing repos.json from redhat-maintenance repo")
        repo.unassociate_unit(['iso'], {'name': "repos.json"}).result()
    ...
    def _unassociate_unit(self, repo, type_ids, criteria):
        ...
        pulp_crit = {
            "type_ids": type_ids,
            "filters": {
                "unit":filters_for_criteria(criteria),
                },
            }

        return self._do_unassociate_unit(repo.id, pulp_crit)

    def _do_unassociate_unit(self, repo_id, crit):
        url = os.path.join(
            self._url, "pulp/api/v2/repositories/%s/actions/unassociate/" % repo_id)

        return self._task_executor.submit(
            self._do_request, method="POST", url=url, json=crit
        )
