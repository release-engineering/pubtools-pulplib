...
def test_upload_one_file(client, requests_mocker, tmpdir, caplog):
    """test upload a file to a repo in pulp"""
    logging.getLogger().setLevel(logging.INFO)
    caplog.set_level(logging.INFO)

    client._PULP_CHUNK_SIZE = 20
    ...
    tasks_report = [
        {"task_id": "7744e2df-39b9-46f0-bb10-feffa2f7014b", "state": "finished"}
    ]

    somefile = tmpdir.join("some-file.txt")
    somefile.write("I am the file to be uploaded")
    ...
    client.upload_one_file(str(somefile), repo_id, "txt", {})
    ...
    assert "Created Pulp task: 7744e2df-39b9-46f0-bb10-feffa2f7014b" in messages
    assert "Pulp task completed: 7744e2df-39b9-46f0-bb10-feffa2f7014b" in messages
